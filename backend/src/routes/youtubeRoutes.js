...
router.get('/transcript', async (req, res) => {
  try {
    ...
    try {
      console.log('Trying YouTube Transcript API method first...');
      
      // Call the Python script which now uses YouTube Transcript API with proxy
      const scriptPath = path.join(process.cwd(), 'src', 'transcript_fetcher.py');
      ...
      console.log(`Using Python executable: ${pythonExecutable}`);
        ...
        throw new Error(result.error || 'Failed to fetch transcript with YouTube Transcript API');
        }
    } catch (ytError) {
      console.error('YouTube Transcript API method failed:', ytError);
      
      // Try ScraperAPI as fallback
      try {
        console.log('Trying ScraperAPI method...');
        ...
        console.error('ScraperAPI method failed:', scraperError);
        
        // Fall back to yt-dlp as last resort
